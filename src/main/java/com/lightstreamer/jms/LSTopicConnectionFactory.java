...
/**
 * A {@link TopicConnectionFactory} interface implementation that can be used used to create a topic
 * connection with a JMS broker through an embedded JMS Extender client connection.
 */
public class LSTopicConnectionFactory extends LSConnectionFactory
  ...
  /**
   * Creates an {@code LSTopicConnectionFactory} object that can create new {@link TopicConnection}
   * objects.
   *
  ...
  /**
   * Creates an {@code LSTopicConnectionFactory} object that can create new {@code TopicConnection}
   * objects.
   *
   ...
   */
  public LSTopicConnectionFactory(String serverAddress, String jmsConnector) {
   ...
   * @param jmsConnector the name of the JMS connector to use
   */
  public LSTopicConnectionFactory(String serverAddress, String adapterSet, String jmsConnector) {
  ...
  /**
   * Creates a new {@code TopicConnection} with an unspecified user identity.
   *
   * <p>The connection is created in stopped mode. No messages will be delivered until the {@link
   * javax.jms.Connection#start()} method is explicitly called on the {@code TopicConnection}
   * object.
   *
   * <p>If a {@link LSConnectionListener} implementation has been set, its events will be called
   * during connection creation.
   *
   * @return the created {@code TopicConnection}
  ...
  public synchronized TopicConnection createTopicConnection() throws JMSException {
    ...
    LSTopicConnection connection = new LSTopicConnection(lsClient, getJmsConnector(), true);
    return connection;
  }

  /**
   * Creates a new {@code TopicConnection} using the specified user name and password.
   *
   * <p>The connection is created in stopped mode. No messages will be delivered until the {@code
   * Connection.start()} method is explicitly called on the {@code TopicConnection} object.
   *
